...
/**
 * Path that refers to the roi-content from the "@angular/components-examples" package. The
 * structure is defined in the Material repository, but we include the roi-content as assets in
 * in the CLI configuration.
 */
const DOCS_CONTENT_PATH = '/roi-content/examples-source';
...
export class StackBlitzWriter {
   ...
   * "<material-roi-example>" and "MaterialDocsExample".
  ...
  private _replaceExamplePlaceholders(
    ...
    if (fileName === 'src/index.html') {
      ...
      // For example, <material-roi-example></material-roi-example> will be replaced as
      // <button-demo></button-demo>
      fileContent = fileContent
        .replace(/material-roi-example/g, data.selectorName)
      ...
      // Replace `import {MaterialDocsExample} from 'material-roi-example'`
      ...
      fileContent = fileContent.replace(/material-roi-example/g, importFileName);
    }
    return fileContent;
  }
