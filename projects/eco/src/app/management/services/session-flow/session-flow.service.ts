...
export class SessionFlowService {
  ...
  constructor(
    ...
    @Inject(PLATFORM_ID) private isBrowser: Object,
    @Inject('sessionId') private sessionId: string,
    @Inject('deviceId') private deviceId: string,
    @Inject('isMobile') private isMobile: boolean,
    ...
    this.userId = 'guest';
    if (isPlatformBrowser(this.isBrowser)) { // Check if it's a browser platform
  ...
  createVisitedRoute() {
    const visitedRoute = new VisitedRoute(this.lastRoute, this.lastRouteTimeStart, new Date());
    visitedRoute.calculateDuration();
    this.visitedRoute.next(visitedRoute);
  }
  ...
  private setCookie(cname: string, cvalue: string, exdays: number) {
    if (this.isBrowser) {
      ...
      document.cookie = cname + '=' + cvalue + '; ' + expires + '; path=/'; // Include the 'expires' and 'path' attributes
    }
  }
  /**
   * Get cookie
  ...
  addUserClick(e: any) {
    ...
    let selectorName = 'undefined';
    for (let i = 0; i < targetValues.length; i++) {
      const targetVal = targetValues[i];
      if (e.target[targetVal]) {
        if (typeof e.target[targetVal] === 'object') {
          selectorName = e.target[targetVal].name;
        } else {
          selectorName = e.target[targetVal];
        }
        break;
      }
    }
    ...
    const userClick = new UserClick(time, selectorName, route);
    this.click.next(userClick);
  }
}
