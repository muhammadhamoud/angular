...
export class ProductService{
  ...
  public esIndex = 'firebase';
   ...
   constructor(private dal: DbAbstractionLayerService){
  }
   ...
   */
  searchProducts(categoryId: string, currentPrice: number, attributes: any[], tags: any[], size: number, from: number){
    let queryObject = {
     size: size,
     from: from,
     query: {
   ...
   */
  getPriceRanges(categoryId: string){
    let queryObject = {
     aggs : {
   ...
   * @returns {Observable} Observable of products
   */
  filterProducts(priceRange: number[], attributes : any[], tags: any[], size: integer, offset: integer){
    let queryObject = {
     size: size,
     from: offset,
     query: {
   ...
   */
  getGeneralCategory(generalCategoryId: string){
    let queryObj = {
        "query": {
            "term":{
   ...
   */
  getProductsByCategoryIds(categoryIds: any[], size: number, from: number){
    let queryObj = {
      size: size,
      from: from,
      query: {
   ...
   */
  getTotalPages(categoryId: string, currentPrice: any[], attributes: any[], tags: any[]){
    let queryObject = {
     size: 0,
     query: {
        bool: {
   ...
   * @returns {Observable} Observable of total items
   */
  getTotalPagesByCategoryIds(categoryIds: any[]){
    let queryObj = {
      size: 0,
      query: {
        constant_score: {
   ...
   */
  getOneAttribute(attributeId: string){
    let queryObj = {
        "query": {
            "term": {
                "_id": attributeId
